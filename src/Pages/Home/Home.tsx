      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="  border border-[#ddd] rounded-[10px]">
          ...
          <div className="cards-list px-4">
            {!isLoading && fiveQuizzesData?.length >= 0
              ? fiveQuizzesData?.map(
                  ({
                    _id,
                    title,
                    ...
                    <div
                      className="card my-2 border border-1 hover:bg-gray-100 border-[#ddd] rounded-[10px]  w-full flex "
                      style={{
                    ...
                    >
                      ...
                      <div className="card-des w-[100%]  p-3">
                        ...
                        <div className="text-[#777]">
                          ...
                          <span>{moment(schadule).format("Do MMM YY")}</span>
                        </div>
              ...
              : Array(3)
                  ...
                  .map((_, index) => (
                    <div
                      key={index}
                      className="flex w-full items-center cards-list ps-0 border border-[#ddd] rounded-[10px] py-0 my-1 overflow-hidden"
                    >
        ...
        <div className=" border border-[#ddd] rounded-[10px]">
          ...
          {!isLoading && fiveStudentsData?.length >= 0
            ? fiveStudentsData?.map(
                ({ _id, first_name, last_name, avg_score, group }) => (
                  <>
                    <div
                      key={_id}
                      className="cards-list px-4"
                      onMouseEnter={() => setHoveredCardId(_id)}
                      onMouseLeave={() => setHoveredCardId(null)}
                    >
                      <div
            ...
            : Array(3)
                ...
                .map((_, index) => (
                  <div
                    key={index}
                    className="flex w-full items-center cards-list ps-0 border border-[#ddd] rounded-[10px] py-0 my-2 overflow-hidden"
                  >
                    ...
                    {isLoading ? (
                      ...
                      <p>
                        {header === "title"
                          ...
                          : header === "schadule"
                          ? ` ${
                              quizzesData?.schadule?.split("T")[0]
                            } -- ${quizzesData?.schadule
                              ?.split("T")[1]
                              .split(".000Z")}`
                          : quizzesData?.status}
                      </p>
