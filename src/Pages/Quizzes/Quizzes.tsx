      ...
      <div className="card my-2 border border-1 border-[#ddd] rounded-[10px] flex flex-col p-4">
        ...
        {!loading ? (
          incomingQuizzes?.slice(0, 2).map((item) => (
            <div
              className="flex items-center cards-list ps-0  border border-[#ddd]  rounded-[10px] py-0 my-1 overflow-hidden hover:bg-gray-100"
              style={{
            ...
            >
              ...
              <div className="card-des w-full p-3">
                ...
                <div className="text-[#777]">
                  <span>{moment(item?.createdAt).format("Do MMM YY")}</span> |{" "}
                  <span>{moment(item?.schadule).format("HH:mmA")}</span>
                </div>
