...
const SetNewQuizModal = ({ openModal, setOpenModal }) => {
  ...
  const handelFormData = async (data) => {
    console.log(data);
    try {
      const res = await dispatch(getAllJoinData(data));
      setOpenModal(false);
  ...
  return (
    ...
    >
      ...
      :
      <>
       <Modal.Header className="p-4 capitalize"></Modal.Header>
      <h2 className="text-center text-3xl font-bold p-5">Join Quiz</h2>
      <p className="text-center text-black">Input the code received for the quiz below to join</p>
      </>
    }
      <Modal.Body>
        {Role=="Instructor"?
